...
import { generateMoons } from '../../../lib/datasets';

export default function ClassifierPage() {
  const [cfg, setCfg] = useState({ lr: 0.1, epochs: 200, hidden: 16, activation: 'tanh', seed: 5 });
  const [dataCfg, setDataCfg] = useState({ n: 140, noise: 0.1 });
  const modelRef = useRef(null);
  const [losses, setLosses] = useState([]);
  const dataset = useMemo(() => generateMoons(dataCfg.n, cfg.seed, dataCfg.noise), [cfg.seed, dataCfg]);
  ...
  return (
    <div>
      <h1>2D Moons Classification</h1>
      <p className="small">Small MLP learns a nonlinear decision boundary on synthetic moons.</p>
      <Controls
        defaults={cfg}
        onChange={setCfg}
        onAction={(act) => {
